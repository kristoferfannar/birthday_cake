...
def binary_search_endpoint(
    ...
    for iteration in range(max_iterations):
        ...
        for direction in [-1, 1]:
            ...
            test_point = move_point_along_perimeter(moving_point, piece, test_distance, direction)
            ...
            valid, area_diff, ratio_diff = get_areas_and_ratios(
                cake, fixed_point, test_point, 0.5, target_area, original_ratio,
                acceptable_area_error, acceptable_ratio_error
            )
...
def refine_cut_binary_search(
    ...
    refined_p1, score1 = binary_search_endpoint(
        cake, p2, p1, piece, target_area, original_ratio,
        max_search_distance, max_iterations, acceptable_area_error, acceptable_ratio_error
    )
    ...
    refined_p2, score2 = binary_search_endpoint(
        cake, p1, p2, piece, target_area, original_ratio,
        max_search_distance, max_iterations, acceptable_area_error, acceptable_ratio_error
    )
    ...
    if best_score < float('inf'):
        final_p1, score_final = binary_search_endpoint(
            cake, best_p2, best_p1, piece, target_area, original_ratio,
            max_search_distance / 2, max_iterations // 2, acceptable_area_error, acceptable_ratio_error
        )
...
def find_valid_cuts_with_refinement(
    ...
    if not top_candidates:
        print(f"Refinement: No coarse cuts found, returning empty list")
        return []
    ...
    for xy1, xy2, piece, _ in top_candidates:
        ...
        refined_p1, refined_p2, refined_score = refine_cut_binary_search(
            cake, xy1, xy2, piece, target_area, original_ratio,
            acceptable_area_error, acceptable_ratio_error
        )
        ...
        if cake.cut_is_valid(refined_p1, refined_p2):
            ...
            if valid:
                ...
                if valid_relaxed:
                    ...
                    refined_cuts.append((xy1, xy2, piece, calculate_optimization_score(area_diff_relaxed, ratio_diff_relaxed)))
    ...
    if skipped_boundary_cuts > 0:
        print(f"Refinement: Skipped {skipped_boundary_cuts} boundary cuts in coarse search")
    
    print(f"Refinement: Found {len(coarse_cuts)} coarse cuts, refined {len(top_candidates)} candidates, got {len(refined_cuts)} final cuts")
