...
def _evaluate_cut_chunk(
    chunk_data: Tuple[List[Tuple[int, Point]], List[Point], Cake, Polygon, float, float, float, float, float]
    ...
    indexed_points1, points2, cake, piece, target_ratio, original_area, original_ratio, acceptable_area_error, acceptable_ratio_error = chunk_data
...
def parallel_find_valid_cuts(
    ...
    if len(perim_points) < 20 or num_workers == 1:
        ...
        return find_valid_cuts(
            cake, perim_points, target_ratio, original_area, original_ratio,
            acceptable_area_error, acceptable_ratio_error
        )
    ...
    chunk_size = max(1, len(indexed_points) // (num_workers * 2))  # Create more chunks than workers
