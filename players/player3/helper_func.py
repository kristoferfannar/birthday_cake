...
def get_areas(cake: Cake, xy1: Point, xy2: Point, target_ratio: float = 0.5, original_area: float = None, acceptable_error: float = 1.0) -> tuple[bool, float | None]:
...
def find_valid_cuts(cake: Cake, perim_points: list[Point] | None = None, target_ratio: float = 0.5, original_area: float = None, acceptable_error: float = 1.0) -> list[tuple[Point, Point, Polygon]]:
    valid_cuts = []
        ...
        for i, xy1 in enumerate(perim_points):
            for xy2 in perim_points[i+1:]:
                ...
                if cake.cut_is_valid(xy1, xy2):
                    areas_valid, area_diff = get_areas(cake, xy1, xy2, target_ratio, original_area, acceptable_error)
